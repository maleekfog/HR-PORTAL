...
const Careers: React.FC = () => {
  ...
  const categories = ['All', 'Design', 'Retail', 'Marketing', 'Operations'];
  ...
  return (
    <div className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
      <header className="mb-24 text-center">
        <h1 className="text-7xl md:text-9xl font-black mb-8 tracking-tighter italic uppercase opacity-90 leading-none">
          CRAFT YOUR <br />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10 border border-white/10 rounded-3xl overflow-hidden">
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => (
            <div key={job.id} className="bg-zinc-950 p-10 md:p-14 hover:bg-white/[0.02] transition-all flex flex-col group">
              <div className="flex justify-between items-start mb-8">
                <span className="text-orange-500 text-[10px] font-black uppercase tracking-widest border border-orange-500/30 px-3 py-1">
                  {job.department}
                </span>
                <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">{job.postedAt}</span>
              </div>
              <h3 className="text-3xl font-bold mb-4 leading-tight uppercase tracking-tight group-hover:text-orange-500 transition-colors">{job.title}</h3>
              ...
              <button 
                ...
                className="w-full bg-zinc-900 border border-white/10 text-white font-black text-xs uppercase tracking-[0.2em] py-5 hover:bg-orange-600 hover:border-orange-600 transition-all"
              >
                Send Portfolio & CV
              </button>
            </div>
          ...
          <div className="col-span-full py-32 text-center bg-zinc-950">
